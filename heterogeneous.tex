...
The aim of this paper consists in validating the following hypothesis: fitting the sub-population size to the computational power of the heterogeneus cluster node can lead to an improvement in run time with respect to the use of the same population size in every node. % gustavo
...
The main question to address in this work is: Is it worth fitting the sub-populations sizes of a dEA to the computational power of every node? To best answer this question a set of experiments has been built with several characteristics in mind:
\begin{itemize}
\item The separate study of two parameter settings: offline and online.
\item Three problems with different characteristics and computational demands have been used.
\end{itemize}

A base execution has been in an homogeneous and heterogeneous clusters. The results of the latter has been used to calculate proportionally the computational power of each node. This 

as a possible offline parameter setting, the computational power
of each node has been calculated proportionally  
to the average number of generations required to find the optimum with the homogeneous parameter set. 

Moreover, a possible way to fit 
...
In a future work, the online adaptation method for sub-population sizes will take into account the nature of the problem at hand. For example, measuring the population diversity along with the generation number. Also, parameters such as migration rate or crossover probability could be fitted to the node's execution characteristics. Other appropriate benchmarks to analyze the algorithm will be used to get an online automatic parameter fitting while nodes enter or exit the topology net, or fitting the parameters to the current system load.
