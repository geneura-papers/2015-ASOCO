...
\title{Effect of population size in distributed evolutionary
  algorithms on heterogeneous clusters}
% no studying? o testing?  - JJ
...
% conectividad? ¬øLos par√°metros de exploraci√≥n y explotaci√≥n? - JJ
Since the dEAs include a whole range of parameters with an influence in the performance, 
this paper proposes a study on the size of the population. This parameter  is one of the most important, 
...
an improvement in running time.  % with respect to... JJ
...
been used. %why two and not three? - JJ
Results show
that setting the population size according to the computational power
of the nodes in the heterogeneous cluster improves the time required
to obtain the optimal solution. % Spoileeeer! - JJ 
...
% lo de arriba sigue sin contestar - JJ
...
%ARGUMENTO: 
% 1) Existen nuevas tendencias que usan nodos heterogÈneos
% 2) Adaptar par·metros de un algoritmo (general) a la potencia de
% esas m·quinas heterogÈneas puede hacer que mejore el tiempo de
% ejecuciÛn 
% ¬øPor qu√©? ¬øY por qu√© quieres mejorar el tiempo de ejecuci√≥n? - JJ
...
% øNodos heterogÈneos? øComputaciÛn distribuida? Pues al principio - JJ FERGU: adapt·ndome al ARGUMENTO (arriba) (Nota, la intro la escribÌ antes de que me revisaras la tesis, asÌ que por eso no est· bien hilado :)

%FERGU2: RE-estructurada de nuevo

% -> y aquÌ explicas lo que son. O pones este p·rrafo como primer
% p·rrafo para enmarcar el trabajo, o lo abrevias y lo pones arriba
% donde lo menciones - JJ FERGU: Reestructurada la intro y movido este p·rrafo al principio
...
individual. This function usually describes the problem being
solved. % usually? - JJ
...
adapted to increase its performance. Among them,
the population size in EAs
 is the a key factor in the performance, because it has an effect on the quality of the
solution and the running-time
\cite{ShrinkageLaredo09}. % is it the only one? Which are the other
                          % parameters? Is it the one with the most
                          % influence? - JJ
This parameter has been studied as a fixed
...
%Estabas hablando de computaciÛn distribuida y ahora hablas de algo
%totalmente diferente. Cada p·rrafo va por su lado. No hay una
%narrativa.
% Escribe tu argumento en tres o cuatro frases y desarrolla ese
% argumento, cada frase en un p·rrafo. La argumentaciÛn debe quedar
% clara. - JJ FERGU: reestructurando la intro
% Mejor estructurada, pero falta motivaci√≥n - JJ
Distributed EAs run the algorithm in a set of nodes simultaneously,
...
model with a ring topology. % Este p√°rrafo deber√≠a ir antes de que
                            % digas qu√© es lo que vas a adaptar - JJ
...
set of parameters for different problems. % Invierte el orden de estas
                                % dos frases - JJ
...
virtual machine instances, % cita a Una-May en EvoStar aqu√≠ - JJ
...
\cite{HETEROGENEOUSHARD} that dEAs with the same parameter
...
exploitation/exploration rate of the search space. % Ein? Sez who? FERGU: Alba en el artÌculo de arriba
                                % Why? - JJ
% pon la cita de nuevo - JJ
...
reduced. When having different processors with different computational
...
assigned with smaller populations. % smaller diversity? - JJ
...
sub-populations. % because... - JJ

In this work, a heterogeneous distributed system has been used to
answer  the following research questions:
\begin{itemize}
 \item Can a distributed EA be fitted to leverage the capability of a
...
\subsection{Hardware and parameter configurations}
 ...
 % øPor quÈ? øCubren el objetivo
                                % del artÌculo? Si no, siempre llegar·
                                % el revisor que te diga "Prueba, no
                                % sÈ, "migraciÛn de poblaciones
                                % completas" o vaya ustÈ a saber... -
                                % JJ FERGU: explicado con la siguiente frase

The main objective of this work is testing parameter fitting to hardware. Thus, different configurations should be used to compare and validate if the change in the parameters depends only on the parameters, the hardware heterogeneity, or the combination of both.
...
\item AdSi/HeHa: Adaptive Size/Heterogeneous Hardware. Online fitting of sub-population sizes in each island on a heterogeneous cluster.
\end{itemize}
...
To study the scalability, two different number of nodes have been used in each problem/configuration: 4 nodes (from H1 to H4, referred as  {\em 4X} configuration for the rest of the paper) and 8 nodes (from H1 to H8, referred {\em 8X} configuration). The same amount of nodes been previously used in other works on dEAs in heterogeneous environments \cite{HETEROGENEOUSHARD,HYDROCM,Salto13Distributed}.
...
In this configuration, each node has the same number of individuals, so the total amount is 1024.  The algorithm has been run 40 times per problem instance on the homogeneous (HoHa) and heterogeneous clusters (HeHa).  The average number of generations in each node of the HeHa cluster are shown in Table \ref{table:generations4X} (for 4X) and Table \ref{table:generations8X} (for 8X). Note how the generations attained and their proportion in every node to reach the optimum depends on the problem considered, besides the hardware.
...
\caption{Average number of generations (and proportion) in each node
  of the 8X HoSi/HeHa configuration when the optimum is found in 40
...
parameters fitted to their power. % REVISAR TODO EL INGL√âS. Cada
...
the HeSi/HeHa and HeSi/HoHa configurations. This has been done by
dividing the total number of individuals (1024) proportionally to the
number of generations spent by each node in HoSi/HeHa to solve the
...
executed (inside a node). 

%EXPLAIN how you adapt size in homogeneour clusters - JJ
...
(reaching the optimum) \cite{EVALUATIONPARALLEL}.  This work is
focused on the third one. % ¬øY las preguntas de investigaci√≥n? - JJ
...
results. % Pero no acabas de decir que es misleading? - JJ
It is difficult to compare the performance of HoHa and HeHa
...
% probar cÛmo funciona en algoritmo. Se pueden comentar los dos juntos
% y se deberÌa hacer - JJ FERGU: okis, poniÈndolo todo junto.
...
the comparison with the base configuration of each cluster (HoSi/HeHa
and HoSi/HoHa respectively) is significantly higher (\ding{115}),
...
lower results, the better. % Deber√≠as expresarlo en t√©rminos de mejor o
                          ...
                          % evaluations is worse or better? - JJ
...
%No se puede poner apaisado en una p√°gina? Esto no se ve nada - JJ
...
\caption{Time (ms) and evaluations needed to obtain the optimum for the MMDP problem.}
...
\caption{Time (ms) and evaluations needed to obtain the optimum for the OneMax problem.}
...
\caption{Time (ms) and evalutions needed to obtain the optimum for the
  Rosenbrock function problem.}
% ¬øQu√© significa el color gris? - JJ 
...
Only the 8X graphs have been shown in order to not extend too much the paper extension. %TODO ponemos que el 4X shows similar behaviours?
...
On the contrary, for the MMDP and Rosenbrock, in the HoHa system,
...
algorithm's performance. % because of... diversity? - JJ

Finally, the number of nodes affect the behaviour of the
algorithms. In the case of MMDP in the HeHa system, the offline
...
% No dices NADA sobre el "control", que es el HOHA. Si cambiar el
...
%}
\subfigure[300 MMDP]{
    \includegraphics[width =6cm] {graphs/mmdp-size-300-mut-0.003-xover-1-heterohardware-adaptsize.eps}
...
\subfigure[15000 OneMax]{
    \includegraphics[width =6cm] {graphs/onemax-size-15000-mut-0.00006-xover-1-heterohardware-adaptsize.eps}
...
\subfigure[50 Rosenbrock]{
    \includegraphics[width =6cm] {graphs/cec-size-50-mut-0.02-xover-1-heterohardware-adaptsize.eps}
...
the lack of space, % no puedes hablar de esto aqu√≠! - JJ
 the 4X configuration for the smallest instance of each problem is shown. Figure \ref{fig:bars} graphically compare these results. As can be seen, the migration is the most time consuming operation in all configurations, being the migration in HeHa more expensive than in HoHa.
%TODO Comprobar esto!!
This happens because a multi-purpose laboratory network to communicate
the nodes is being used, instead of the specific one used in the HoHa
...
The objective of this paper has been to study if fitting the
...
time. Two parameter setting schemes (offline and online) that use
information of the computational load of the algorithm have been
tested, and three different problems that cover different
characteristics and computational demands have been used. In this
work, as a possible offline parameter setting, the computational power
...
number between each node and its previous neighbour. % research
 				...
 				% No. Tratamos de probar un objetivo, dilo aquÌ!!!!! FERGU: puesto el objetivo, reordenadas conclusiones
...
to keeping
the same size for all nodes. This reduction is due to the combination
...
machines, %argh. Eso es obvio - JJ
 ...
 space. % eso lo has probado o lo supones? Lo has medido? - JJ
...
considering the same parameter value in all nodes. % Reescribe. Todo
                                % el p√°rrafo est√°
                                % mal escrito y no se entiende- JJ
...
Furthermore, changing the sub-population size affects stages
of the algorithm that are independent of this parameter, such as
the migration. The sub-population size fitting is also affected by the problem being solved. These results are a promising starting for fitting EAs to the
performance of each  node, using more suitable benchmarks or even in a dynamic way. 
