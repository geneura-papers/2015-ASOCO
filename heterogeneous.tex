...
% Hay un salto desde "heterog√©neos" hasta "voy a alterar la
% poblaci√≥n". Tendr√°s que recorrerlo con m√°s cuidado, porque ahora no
% se ve. ¬øPor qu√© la poblaci√≥n y no la frecuencia de inmigraci√≥n? ¬øLa
% conectividad? ¬øLos par√°metros de exploraci√≥n y explotaci√≥n? - JJ FERGU: lo de exploration factor abajo. øCÛmo sugieres?
...
The aim of this paper consists in validating the following hypothesis: fitting the sub-population size to the computational power of the heterogeneous cluster node can lead to an improvement in running time with respect to the use of the same population size in every node. % gustavo
%presents an improvement in running time with respect to use the same population sizes in all nodes. % with respect to... JJ FERGU: Done
...
been used. %why two and not three? - JJ FERGU, esto estaba de antes, lo arreglÈ diciendo lo de distintas caracterÌsticas
...
% Por favor, mucho cuidado con el argumento aquÌ, porque no est· nada
% claro por dÛnde va. Hay que enmarcar claramente el problema y por
% quÈ a la segunda frase del abstract estamos hablando de la poblaciÛn
% En todo caso, en la introducciÛn habr· que dejar claro por quÈ nos
% centramos en la poblaciÛn y no en otras cosas. øQuÈ estamos
% buscando? øQue se sincronicen todos los nodos? Si es asÌ hay que
% decirlo arriba. - JJ FERGU: No, no se busca una sincronizaciÛn tal cual, escribo al principio de la intro un argumento y desarrollarÈ desde allÌ.
% El argumento sigue sin estar claro. ``Estamos preocupados con AEs
% distribuidos... donde la poblaciÛn es muy importante''. øQuÈ tiene
% esto que ver con los sistemas heterogÈneos? Para empezar, el marco
% deberÌa estar en la primera frase: estamos hablando de algoritmos
% evolutivos distribuidos. Para seguir, el problema es usar todos los
% recursos. Para usar todos los recursos, debemos øquÈ? øQue los nodos
% funcionen de forma sÌncrona? øQue las prestaciones sean similares?
% øQuÈ tiene que ver esto con las prestaciones= Y para hacer eso, lo m·s importante es
% controlar la poblaciÛn... Por favor, dÈjalo bien claro porque la
% lÌnea de argumentaciÛn es lo m·s importante del paper - JJ

% lo de arriba sigue sin contestar - JJ FERGU: lo del orden de arriba lo habÌa cambiado a como sugerÌas: Primero AEs, luego distribuidos, luego los recursos, pero meto abajo lo de sincronismo y asincronismo
...
% the steps taken to make it sure too - JJ  FERGU: el objetivo del paper es reducir tiempo de ejecuciÛn. Si aumentamos las evaluaciones, pero tardamos menos tiempo (como sucede en este paper), pues mejor, antes nos vamos a casa. Ese tiempo se aprovecha mejor dandole tamaÒos pequeÒos a nodos lentos, porque explotan, pero no demasiado r·pido.
 ...
 %Two different things:
                                ...
                                %algorithm. - JJ FERGU: el n˙mero de evaluaciones no es importante que decrezca en un sistema heterogÈneo.
...
solved. % usually? - JJ FERGU: lo leÌ de alguna tesis, pero lo quito

% How do you define performance? Entre uno y otro p√°rrafo deber√≠a
% haber otro que dijera: The performance of an evolutionary algorithms
% is measured by... - JJ FERGU: okis, reescrito abajo el primer p·rrafo y enlazado con dEAs
...
model with a ring topology. % Este p√°rrafo deber√≠a ir antes de que
                            % digas qu√© es lo que vas a adaptar - JJ FERGU: okis, movido de sitio
...
computational capabilities of a heterogeneous cluster. % but why
                                ...
                                % cada cosa en su sitio y motivando el siguiente p·rrafo                              
...
virtual machine instances, % cita a Una-May en EvoStar aqu√≠ - JJ FERGU: Citada arriba con Derby13Cloud
...
more efficient in time and number of evaluations in reaching the optimal solution on this kind of environments  than on clusters with
...
exploitation/exploration rate of the search space \cite{HETEROGENEOUSHARD}. % Ein? Sez who? FERGU: Alba en el artÌculo de arriba
...
assigned with smaller populations, having smaller diversity. % smaller diversity? - JJ FERGU: Puesto (con Carlos aquÌ)
...
as slower nodes can exploit solutions without premature convergence and,
 at the same time, benefit from the diversity of the faster ones. % because... - JJ FERGU: puesta la ˙ltima frase, mira a ver si te mola
...
%Hasta aquÌ no has dicho nada de que lo que vas buscando es una
%sincronizaciÛn de los nodos o, para el caso, si lo que usas es un
%algoritmo sÌncrono o asÌncrono. Lo que ser· importante luego para
%explicar los resultados. - JJ FERGU: puesto lo de asincrono aquÌ, y m·s abajo
...
%OJO! En AdaptationSizesSchlierkamp96 dicen que cuando no ponen tamaÒo global fijo, el tamaÒo de las subpoblaciones y los resultados son menos claros (usarlo m·s alante)
...
In this study, a dEA with the same sub-population sizes  has been executed in the heterogeneous cluster until the optimal solution is found, as a baseline. Then, the obtained number of generations have been used, to proportionally distribute the number of individual among the nodes, that is, offline. The same set of sizes is used in an homogeneous cluster (as control system) to validate if the changes in performance are due to the parameters or the fitting to the nodes. Finally, an online parameter setting that extract relative information about the performance of the nodes has been tested to validate our approach. %TODO esto de validate our approach no mola, cambiarlo tambiÈn en las contestaciones a los revisores
...
%algoritmo, cambiarÌan los resultados? øPor quÈ se ha usado
...
%deberÌan ser m·s o menos independientes del problema, pero se han
...
% ponerlo como sz que si.- JJ FERGU: Okis, lo cambio. Sin embargo, dejo el Size como 'si', que lo veo m·s natural (y para no cambiar las gr·ficas)
...
To study the scalability, two different number of nodes have been used in each problem and configuration: 4 nodes (from H1 to H4, referred as  {\em 4X} configuration for the rest of the paper) and 8 nodes (from H1 to H8, referred {\em 8X} configuration). The same amount of nodes been previously used in other works on dEAs on heterogeneous environments \cite{HETEROGENEOUSHARD,HYDROCM,Salto13Distributed}.
...
% Deber√≠ais poner n√∫mero de evaluaciones adem√°s. La relaci√≥n ser√≠a
% mas exagerada - JJ FERGU: si ponemos evaluaciones (que es simplemente Gen*128 o 256) salen n˙meros con m·s dÌgitos y creo que es m·s difÌcil de comparar
...
cluster nodes presents an improvement in running time.  % yet you also
                                ...
                                % objective of that experiment? - JJ FERGU: he puesto lo de control arriba al principio de la secciÛn y abajo
...
% REVISAR TODO EL INGL√âS. Cada
                                % p√°rrafo que reviso tiene uno o dos
...
%EXPLAIN how you adapt size in homogeneour clusters - JJ FERGU: en el HoHW no adaptamos el tamaÒo al nodo porque son los nodos iguales, pero para una comparativa justa, usamos los mismos par·metros en ambos sistemas (puesto arriba)
...
% Aqu√≠ tienes que contestar a las preguntas de investigaci√≥n que has
% planteado en la intro - JJ FERGU: contest·ndolas
...
focused on the third objective as it is the most adequate to solve the second research question presented in the Introduction. % ¬øY las preguntas de investigaci√≥n? - JJ FERGU: Puestas en el texto donde corresponde en varios sitios
...
% No me he enterado de nada. ¬øNo se puede reflejar la velocidad pero
...
lower results, the better. % Deber√≠as expresarlo en t√©rminos de mejor o
                          % peor, no higher o lower. ¬øhigher number of
                          % evaluations is worse or better? - JJ FERGU: no puedo, en el paper siempre nos centramos en "menos tiempo es mejor", pero las evaluaciones dan informaciÛn para saber quÈ est· pasando, por lo que menos evaluaciones no tiene por quÈ ser mejor
...
%No se puede poner apaisado en una p√°gina? Esto no se ve nada - JJ FERGU: TODO espero a ver quÈ me dicen los maquetadores de la revista
...
% ¬øQu√© significa el color gris? - JJ FERGU: lo explico
...
among the problems.  % so what? - JJ FERGU: puesta la razÛn
...
algorithm's performance. Homogeneous nodes exploring different population sizes at the same speed implies less diversity during the runs. This can be seen in Figures \ref{fig:graphsMMDP} and \ref{fig:graphsROSENBROCK}, where in all the instances of those problems, the average population fitness in HoHW is closer to the best average value attained. This behaviour is opposed to the same problems executed on HeHW, where the difference of the average best fitness and the average population fitness is larger. This difference is more large at the beginning of the runs (local optima are found faster) but the diversity is still maintained, and being reduced with time.

% because of... diversity? - JJ FERGU: enlanzando a las gr·ficas y explic·ndolas
...
%Maribel, esto va a ser duro de explicar, quiz·s porque al haber m·s nodos, hay muchos m·s individuos y por lo tanto m·s diversidad y la diversidad es clave en este problema porque si no hay bloques buenos, da igual el mucho tiempo que lo evoluciones, que no se llegan a conseguir, pero con muchos individuos, tienes mucha diversidad y tienes disponibles bloques buenos que van combin·ndose mucho m·s r·pido.
...
% tama√±o de la poblaci√≥n no mejora los resultados (salvo en el
% OneMax), ¬øa qu√© se debe? Debes usar HmHW (le he cambiado el nombre
% ya) como CONTROL y comparar los resultados en uno y otro caso. ¬øCu√°l
% es el factor que hace que los resultados sean, en los casos m√°s
% complicados, mejores? Las tablas 6 y 7 son las fundamentales. - JJ FERGU: puesto cosas del control y explicando m·s las diferencias
...
\centering
%\subfigure[150 MMDP ]{
%    \includegraphics[width =4cm] {mmdp-size-150-mut-0.006-xover-1-heterohardware-adaptsize.eps}
%   \label{subfig:adaptMMDP150}
%}
%\subfigure[240 MMDP ]{
%    \includegraphics[width =4cm] {mmdp-size-240-mut-0.004-xover-1-heterohardware-adaptsize.eps}
...
%    \includegraphics[width =4cm] {onemax-size-5000-mut-0.0002-xover-1-heterohardware-adaptsize.eps}
%   \label{subfig:adaptONEMAX5000}
%}
%\subfigure[10000 OneMax]{
%    \includegraphics[width =4cm] {onemax-size-10000-mut-0.0001-xover-1-heterohardware-adaptsize.eps}%
...
%    \includegraphics[width =4cm] {cec-size-10-mut-0.1-xover-1-heterohardware-adaptsize.eps}
%   \label{subfig:adaptROSENBROCK10}
%}
%\subfigure[30 Rosenbrock]{
%    \includegraphics[width =4cm] {cec-size-30-mut-0.03-xover-1-heterohardware-adaptsize.eps}%
...
and HeSi) is taken into account. For the shake of clarity in results,  % no puedes hablar de esto aqu√≠! - JJ FERGU: Ok, quitado
...
%Habr√≠a que hacer un r√°nking de m√©todos para hardware heterog√©neo y
%homog√©neo. No hay un "smoking gun", no hay una conclusi√≥n fuerte. Al
%final, la pregunta es: tengo hardware heterog√©neo, ¬øqu√© diablos uso?
% Por otro lado, tambi√©n es importante considerar si el beneficio de
% diferente exploraci√≥n de nodos heterog√©neos es real o no. ¬øPara qu√©
% has probado hardware homog√©neo? Para ver si la poblaci√≥n diferente
% trae alg√∫n beneficio en la exploraci√≥n del sistema. Bueno, pues
% resulta que NO. En la mayor parte de los casos, es PEOR resultado
% que si el tama√±o es homog√©neo. Por lo tanto, si la poblaci√≥n
% heterog√©nea no trae mejores resultados, ¬øqu√© es lo que  lo hace? ¬øLa
% asincron√≠a? - JJ FERGU: puesto abajo en las conclusiones lo de la asincronia

\section{Conclusions and future work}

%Different research questions have been proposed in this paper to study if fitting the sub-population size of a distributed EA to the computational power of a heterogeneous cluster nodes presents an improvement in running time. To answer these questions two parameter setting schemes (offline and online) that use information of the computational load of the algorithm have been tested, and three different problems that cover different characteristics and computational requirements have been used.

The main question to address in this work is: Is it worth fitting the 
...
experiments has been built with several characteristics in mind. 
...
A base configuration with the same sub-population sizes has been used on a heterogeneous cluster. The results have been used to calculate offline the computational power of each node, comparing the average number of generations to find the optimal solution. These proportions are used to set the new sub-population sizes. Also, a possible way to fit online the sub-population sizes has been performed comparing the current generation number between each node and its previous neighbour in the topology. % research
                                % questions!!!!! - JJ FERGU: poniÈndolas y contest·ndolas en las conclusiones

Results show that online or offline fitting the sub-population size to the computational power of each node in a heterogeneous cluster
can reduce significantly the running time of an algorithm with respect
to keeping the same size for all nodes. The sub-population size fitting is also affected by the problem being solved. Also, to answer the question about the difference of using these same sub-populations sizes in an homogeneous cluster, a control system has also been used for comparison. In this case, the same parameter sets used in an homogeneous cluster do not improve the running time in multi-modal problems, as the average fitness of the population remains close to the average best fitness during all the evolution, and therefore, fitness diversity is not as well maintained as in the heterogeneous cluster.
...
%considering the same parameter value in all nodes. % Reescribe. Todo
                                % el p√°rrafo est√°
                                % mal escrito y no se entiende- JJ FERGU: reescribo y lo pongo relacionado con las questions arriba tambiÈn


 % o sea, tener
                                % maquinas heterogÈneas y par·metros
                                % heterogÈneos es mejor porque usamos
                                % par·metros heterogÈneos en m·quinas
                                % heterogÈneas. Di en quÈ puede
                                % influir eso en la mejora de los
                                % resultados y discute por quÈ podrÌa
                                % ser asÌ y propÛn experimentos para
                                % probar que efectivamente se trata de
                                % eso. - JJ FERGU: puestos nuevos experimentos y explicado
% No se ha explicado lo suficiente. Las conclusiones deben ser m√°s
% taxativas y apoyadas en datos de los experimentos. Si hay alguna
% suposici√≥n que no se pueda comprobar, se deja como trabajo futuro -
% JJ FERGU: reescritas, mira a ver si ahora est· m·s claro
...
% Falta una discusiÛn sobre si las mejoras se deben exclusivamente al
% n√∫mero de evaluaciones o hay alg√∫n otro factor ømenos overhead?
% ønodos m·s r·pidos? - JJ FERGU: no, de hecho el
% n√∫mero de evaluaciones no siempre disminuye, lo digo arriba.
% Entonces, a qu√© se debe la mejora? Si hay m√°s evaluaciones, el
% algoritmo es peor - JJ

%In future works, new methods to adapt online the sub-population sizes taking into account the nature of the problem will be used (for example, measuring the population diversity at the same time than the number of generations). Also, other parameters such as migration rate or crossover probability could be fit to the execution nodes. Other appropriate benchmarks to analyse the algorithm will be also used to lead to automatic parameter fit in runtime (online), with different nodes entering or exiting in the topology, or fitting the parameters to the current load of the system.
...
This work has been supported in part by SIPESCA (Programa Operativo FEDER de AndalucÌa 2007-2013), TIN2011-28627-C04-02 and TIN2014-56494-C4-3-P (Spanish Ministry of Economy and Competitivity), SPIP2014-01437 (Direcci{\'o}n General de Tr{\'a}fico), PRY142/14 (Fundaci{\'o}n P{\'u}blica Andaluza Centro de Estudios Andaluces en la IX Convocatoria de Proyectos de Investigaci{\'o}n), and PYR-2014-17 GENIL project (CEI-BIOTIC Granada). Authors wish to thank reviewers' comments, whose suggestion and guidelines have contributed to improve this work.
...
% Ten√©is que revisar la bibliograf√≠a. Por ejemplo, faltan muchos
% editores en proceedings. Usad la de DBLP y la est√°ndar de
% geneura. Por ejemplo, pones Guerv√≥s sin acento, Garc√≠a tambi√©n y en
% uno pones Gu√©rvos!!!!- JJ FERGU: Corregida la bibliografÌa entera (nota, el Guervos ese es oficial del bib de la Soft Computing...)
