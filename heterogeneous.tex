...
% conectividad? Â¿Los parÃ¡metros de exploraciÃ³n y explotaciÃ³n? - JJ
...
% 2) Adaptar parámetros de un algoritmo (general) a la potencia de
% esas máquinas heterogéneas puede hacer que mejore el tiempo de
% ejecución 
% Â¿Por quÃ©? Â¿Y por quÃ© quieres mejorar el tiempo de ejecuciÃ³n? - JJ
...
% Mejor estructurada, pero falta motivaciÃ³n - JJ
...
model with a ring topology. % Este pÃ¡rrafo deberÃ­a ir antes de que
                            % digas quÃ© es lo que vas a adaptar - JJ
...
virtual machine instances, % cita a Una-May en EvoStar aquÃ­ - JJ
...
The results show that, in general, there exist significant differences depending on the problem, configuration or number of nodes. Also, it can be seen that the number of evaluations may not be a good indicator when comparing heterogeneous environments. For example, for the OneMax problem (Figure \ref{fig:boxplotsONEMAX}), using 8 nodes instead of 4 requires more evaluations to finish, and yet, less time to achieve the optimum. Also, the average amount of time needed to solve the problems may vary in larger ranges for MMDP and Rosenbrock. 

At a first glance, and as expected \cite{Wolpert97NFL}, fitting the population sub-sizes to heterogeneous clusters do not always imply an improvement in time. The HeSi parameter setting can improve time in the MMDP and Rosenbrock for HeHa, but in case of the OneMax problem, fitting offline the sub-population sizes significantly increase  the running time for solving it in the heterogeneous cluster. This can be seen in the average evolution of the OneMax problem in Figure \ref{fig:graphsONEMAX}, in all HeHa configurations the average fitness of the population is located in the lower parts of the boxplots with respect to the best fitness, while in the HoHa the evolution is more close this value. The HeSi and HoSi lines are always moving closer during all the evolution, while the AdSi is obtaining better results in the same time. Also, the best individual of all runs (black line) do not converge as smoothly as in HoHa.
...
On the contrary, for the MMDP and Rosenbrock, in the HoHa system, setting heterogeneous sizes to sub-populations makes no difference in time, that is, changing this parameter has no influence in the  algorithm's performance.

Finally, the number of nodes affect the behaviour of the algorithms. In the case of MMDP in the HeHa system, fitting offline the sub-population to the computational  power of each node makes the algorithm finish significantly earlier in all instances of the problem in the 4X configuration. However, in the 8X configuration the time is significantly lower only in the 150 instance. This can be due to the nature of this problem, as the population fitness may stagnate if the optimum is not found at the first steps (see the black line in Figure \ref{fig:graphsMMDP} with respect to the average population fitness boxplots). This behaviour is opposite in the case of the Rosenbrock function, where using a larger number of nodes improves the time with HeSi and AdSi. %TODO
...
In this sub-section the time spent by each node of the clusters in every stage of the EA for each configuration with fixed sizes (HoSi and HeSi) is analysed. For the shake of clarity in results, and due to the lack of space, the 4X configuration for the smallest instance of each problem is shown. Figure \ref{fig:bars} graphically compare these results. As can be seen, the migration is the most time consuming operation in all configurations, being the migration in HeHa more expensive than in HoHa.
%TODO Comprobar esto!!
...
The objective of this paper is to study if fitting the sub-population size of a distributed EA to the computational power of a heterogeneous cluster nodes presents an improvement in running time. Two parameter setting schemes (offline and online) that use information of the computational load of the algorithm have been tested, and three different problems that cover different characteristics and computational demands have been used. In this work, as a possible offline parameter setting, the computational power of each node has been calculated proportionally 
...
number between each node and its previous neighbour.

 				% No. Tratamos de probar un objetivo, dilo aquí!!!!! FERGU: puesto el objetivo, reordenadas conclusiones
Results show that fitting online or offline the sub-population size to the computational power of each node in the heterogeneous cluster
can reduce significantly the running time compared to keep
the same parameter value in all nodes. This advantage is due to the combination of the heterogeneous parameters with the heterogeneity of the machines, leading to different exploitation/exploration rates of the search space. On the contrary, the same heterogeneous parameter setting in all islands of the homogeneous cluster could not improve the results than considering the same parameter value in all nodes.
