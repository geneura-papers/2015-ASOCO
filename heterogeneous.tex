...
However, fitting this kind of algorithms to these environments, so that they can take advantage of their heterogeneity to save running time, is still an issue.
The different computational power of the nodes may affect the performance of the algorithm, and a proper tuning or fitting of the algorithm to each node could reduce execution time.
...
to the computational power of the heterogeneous cluster nodes presents an improvement in running time. 
Two size parameter setting schemes have been tested, an offline and an online parameter setting, and three 
problems with different characteristics and computational demands have been used.
...
create a new set of candidates, until a {\em stopping criterion}
...
In distributed EAs the algorithm is run in a set of nodes simultaneously,
...
\begin{itemize}
 ...
 \item How the fitting of the sub-population size to the computational power affect the running time and number of evaluations?
