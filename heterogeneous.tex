...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  SOA  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\section{Background on adaption in heterogeneous clusters}
\label{sec:soa}
...
In other works, there is not direct relation with the algorithm parameters and 
...
Also, to study the scalability, two different number of nodes have been used in each problem/configuration: 4 nodes (from H1 to H4, defined {\em 4X} configuration for the rest of the paper) and 8 nodes (from H1 to H8, defined {\em 8X} configuration). The global population size has been set to 1024 individuals. Other researchers that investigate in dEAs in heterogeneous environments have used 512 individuals (see the works \cite{Dominguez13HydroCM,HYDROCM,Salto13Distributed}). However, using 512 individuals in first preliminary runs obtained extremely low sub-population sizes, so we set a larger value. 1024 individuals is a number big enough to test different sub-population sizes in different number of nodes (4 and 8), allowing an easier distribution among them. The chosen stop criterion along this work is to find the optimum of the problem (so, all runs in this work have always finished when the optimum have been found in one of the nodes). We have used this stop criterion as Alba et al. did in \cite{EVALUATIONPARALLEL}, since it offers a fair way to compare the execution time and behaviour of the configurations to compare. 

\subsubsection{Homogeneous Size configuration}



In this configuration, each node has the same number of individuals (so, the total amount is 1024).  After executing the algorithm 40 times per problem on the heterogeneous cluster, we have obtained the average number of generations in each node, as it can be seen in Table \ref{table:generations4X} (for 4X) and Table \ref{table:generations8X} (for 8X). Note how the generations attained (and their proportion in every node) to reach the optimum depends on the problem considered (besides the hardware).
