...
node. For example, the node HeN4 requires a lot of time in the replacement stage,
 as it is the only one that writes into the log files. This node was the only one
  using a network storage (see Table \ref{tabcomputers}).
...
limited memory (and swapping) requires higher time than the iteration of
...
considering the same parameter value in all nodes. % Reescribe. Está
                                % mal escrito - JJ
