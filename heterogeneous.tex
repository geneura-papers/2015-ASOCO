...
To study the scalability, two different numbers of nodes have been used in each problem and configuration: 4 nodes (from H1 to H4, referred as  {\em 4X} configuration for the rest of the paper) and 8 nodes (from H1 to H8, referred {\em 8X} configuration). The same amount of nodes been previously used in other works on dEAs on heterogeneous environments \cite{HETEROGENEOUSHARD,HYDROCM,Salto13Distributed}.
...
the migration.  These results are a promising star for fitting EAs to the performance of each  node, using more suitable benchmarks or even in a dynamic way. 
